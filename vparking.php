...
<div class="container">
  ...
  if(isset($_POST['search'])){
    ...
    $sql="SELECT * FROM `parking` WHERE CONCAT(`pslot`, `vnum`, `intime`, `outtime`, `date`)LIKE '%".$valuetosearch."%'";
       ...
       <?php
                    $i=0;
                      while($row = mysqli_fetch_array($result)) { 
                         ...
                         $j=0;
                         $date=$row['date'];
                         $new=date('Y-m-d', strtotime($date. ' + 2 days'));
                         $current=date('Y-m-d');
                         if($current >= $new)
                         {
                          $j=$j+1;
                    ...
                    <tr>
                      ...
                      <td><?php $status = $j; if($status=='1') { echo 'EXCEED';} else { echo 'IN-TIME'; } ?></td>
                      <td><?php echo $row ['intime']?></td>
                      <td><?php echo $row ['outtime']?></td>
                      <td><a href="fee.php?pslot=<?php echo $row ['pslot']; ?>&intime=<?php echo $row ['intime'];?>"><i class="fas fa-rupee"></i></a></td>
                      <td><a href="dparking.php?vnum=<?php echo $row ['vnum']; ?>"><i class="fas fa-trash"></i></a></td>
                    </tr>
                    <?php 
                      }
        ?>
 ...
 <div class="text-center">
            <a class="d-block small" href="parking.php">Return To Parking</a>
          </div>
          <br>
</div>
</div>
